...
import {
  ...
  AlertTriangle, Clock, CheckCircle, CheckCircle2, AlertCircle
} from "lucide-react";
import { DashboardLayout } from "../components/layout/DashboardLayout";
import { apiFetch } from "../services/api";
import { getUserId, formatDate } from "../utils/helpers";
import StatusBadge from "../components/ui/StatusBadge";

// ─── inline Button shim (replace with your real Button once it exists) ───────
function Button({ children, variant = "primary", className = "", ...props }) {
  const base = "px-4 py-2 rounded-xl text-sm font-semibold transition-all duration-200";
  const styles = {
    primary: "bg-cs-700 text-white hover:bg-cs-800",
...
// ── fallback data so the page isn't empty before the API resolves ────────────
const FALLBACK_DEADLINES = [
  { id:1, title:"GST Return Filing", description:"Monthly GSTR-3B submission", compliance_type:"GST",        deadline_date:"2026-05-07" },
  { id:2, title:"TDS Deposit",        description:"Q4 TDS payment due",         compliance_type:"TDS",        deadline_date:"2026-05-07" },
  { id:3, title:"ESI Contribution",   description:"Employee State Insurance",   compliance_type:"Labour",     deadline_date:"2026-05-10" },
  { id:4, title:"Advance Tax",        description:"First instalment due",       compliance_type:"Income Tax", deadline_date:"2026-05-14" },
];
// ─────────────────────────────────────────────────────────────────────────────
...
export default function CalendarPage() {
  const [deadlineData, setDeadlineData] = useState(FALLBACK_DEADLINES);
  const [selected, setSelected]         = useState(10);

  useEffect(() => {
    const uid = getUserId();
    if (!uid) return;
    apiFetch(`/api/compliance/calendar/${uid}`)
      .then((data) => { if (Array.isArray(data) && data.length) setDeadlineData(data); })
      .catch(() => {/* keep fallback */});
  }, []);
      ...
      <div className="p-6 lg:p-10 max-w-7xl mx-auto min-h-screen relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-start md:items-end justify-between mb-10 gap-4">
          <div>
            ...
            <p className="text-slate-500 text-base font-medium ml-14">
              Track regulatory deadlines for Q2 2026
            </p>
          </div>
          <div className="flex gap-3">
            <Button variant="outline" className="bg-white/80 backdrop-blur-md shadow-sm hover:bg-slate-50">
              Export CSV
            </Button>
            <Button variant="primary" className="shadow-lg shadow-cs-600/20">
              Add Event
            </Button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-[1fr_400px] gap-8 relative z-10">
          ...
          >
            ...
            <div className="grid grid-cols-7 mb-4">
              {WEEKDAYS.map((d) => (
                <span key={d} className="text-center text-xs font-black text-slate-400 tracking-widest">
                  {d}
                </span>
              ))}
            </div>
                    ...
                    <motion.button
                      ...
                      className={`relative w-12 h-12 sm:w-14 sm:h-14 flex items-center justify-center rounded-2xl text-base font-bold transition-all duration-300
                        ${muted
                          ...
                          : "text-slate-700 bg-slate-50 hover:bg-white hover:shadow-md border border-transparent hover:border-slate-100"}
                        ${isSelected
                          ? "!bg-gradient-to-br !from-cs-700 !to-cs-900 !text-white shadow-xl shadow-cs-900/30 !border-cs-600 scale-110 z-10"
                          : ""}
            ...
            >
              <div className="flex items-center justify-between mb-6 pb-4 border-b border-slate-100">
                ...
                <span className="bg-cs-100 text-cs-700 text-xs font-black tracking-widest px-3 py-1 rounded-full">
                  {(dayEvents.length || deadlineData.slice(0,3).length)} TASKS
                </span>
              </div>
              <div className="space-y-4">
                <AnimatePresence mode="popLayout">
                  {(dayEvents.length ? dayEvents : deadlineData.slice(0, 3)).map((ev, i) => (
                    ...
                    >
                      <div className="absolute left-0 top-3 bottom-3 w-1.5 bg-gradient-to-b from-cs-400 to-cs-600 rounded-full" />
                      <div className="flex items-start justify-between gap-3">
                        <div>
                          <p className="font-bold text-slate-900 text-sm group-hover:text-cs-700 transition-colors">
                            {ev.title}
                          </p>
                          <p className="text-slate-500 text-xs mt-1 leading-relaxed">{ev.description}</p>
                        </div>
                        <span className="bg-white border border-slate-200 shadow-sm text-slate-600 text-[10px] font-bold px-2 py-1 rounded-md flex-shrink-0 uppercase tracking-wider">
                          {ev.compliance_type}
                        </span>
                      </div>
                    </motion.div>
            ...
            >
              {[
                ...
                <div key={label} className="bg-white border border-slate-100 shadow-sm rounded-2xl p-5 hover:shadow-md transition-shadow group">
                  ...
                  <p className={`text-4xl font-extrabold tracking-tight ${color} group-hover:scale-105 origin-left transition-transform`}>
                    {val}
                  </p>
                </div>
              ))}
            ...
            >
              ...
              <div className="relative z-10">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <h2 className="font-bold text-sm tracking-widest uppercase text-slate-300">
                    AI Risk Forecast
                  </h2>
                </div>
                <div className="flex items-end justify-between mb-3">
                  <span className="text-white font-medium text-lg">Late Filing Risk</span>
                  <span className="bg-emerald-500/20 border border-emerald-500/30 text-emerald-300 text-xs font-black px-3 py-1 rounded-full">
                    LOW
                  </span>
                </div>
